...
Generator: SMPT
Application: Reachability query on the instance SmallOperatingSystem-PT-MT0016DC0008 from the Model Checking Contest 2021 using the SMPT model-checker
Target solver: z3
...
(assert (= TaskOnDisk (+ DiskControllerUnit 8)))
...
(assert (= (+ a4 LoadingMem) 8))
(assert (= (+ FreeMemSegment LoadingMem a5) 16))

(assert (not (or (and (or (or (and (>= DiskControllerUnit 1)(>= FreeMemSegment 1)(>= TaskOnDisk 1))(and (>= CPUUnit 1)(>= TaskReady 1)))(and (>= CPUUnit 1)(>= TaskSuspended 1)))(or (not (>= ExecutingTask 1))(or (and (>= DiskControllerUnit 1)(>= TaskOnDisk 1)(>= TaskSuspended 1))(and (>= DiskControllerUnit 1)(>= FreeMemSegment 1)(>= TaskOnDisk 1)))))(not (not (not (and (>= DiskControllerUnit 1)(>= TaskOnDisk 1)(>= TaskSuspended 1))))))))
