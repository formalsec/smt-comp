...
Generator: SMPT
Application: Reachability query on the instance SmallOperatingSystem-PT-MT2048DC1024 from the Model Checking Contest 2021 using the SMPT model-checker
Target solver: z3
...
(assert (= TaskOnDisk (+ DiskControllerUnit 1024)))
...
(assert (and (and (not (or (>= LoadingMem 1)(>= LoadingMem 1)))(and (and (>= TaskReady 1)(and (>= CPUUnit 1)(>= TaskSuspended 1)))(>= TransferToDisk 1)))(or (not (and (>= DiskControllerUnit 1)(>= TaskOnDisk 1)(>= TaskSuspended 1)))(not (or (and (>= CPUUnit 1)(>= TaskSuspended 1))(and (>= DiskControllerUnit 1)(>= TaskOnDisk 1)(>= TaskSuspended 1)))))))
