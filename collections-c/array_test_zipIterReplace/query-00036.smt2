; query-00036.smtml
(set-info :status unknown)
(set-logic ALL)
(declare-fun symbol_8 () (_ BitVec 32))
(declare-fun symbol_7 () (_ BitVec 32))
(declare-fun symbol_1 () (_ BitVec 32))
(declare-fun symbol_3 () (_ BitVec 32))
(declare-fun symbol_2 () (_ BitVec 32))
(declare-fun symbol_0 () (_ BitVec 32))
(assert
 (let ((?x11 ((_ extract 7 0) symbol_8)))
 (let ((?x12 ((_ zero_extend 24) ?x11)))
 (let ((?x14 (bvshl ?x12 (_ bv24 32))))
 (let ((?x15 (bvashr ?x14 (_ bv24 32))))
 (= ?x15 (_ bv0 32)))))))
(assert
 (let ((?x24 ((_ extract 7 0) symbol_7)))
 (let ((?x25 ((_ zero_extend 24) ?x24)))
 (let ((?x26 (bvshl ?x25 (_ bv24 32))))
 (let ((?x27 (bvashr ?x26 (_ bv24 32))))
 (let ((?x19 ((_ extract 7 0) symbol_1)))
 (let ((?x20 ((_ zero_extend 24) ?x19)))
 (let ((?x21 (bvshl ?x20 (_ bv24 32))))
 (let ((?x22 (bvashr ?x21 (_ bv24 32))))
 (and (distinct ?x22 ?x27) true))))))))))
(assert
 (let ((?x19 ((_ extract 7 0) symbol_1)))
 (let ((?x20 ((_ zero_extend 24) ?x19)))
 (let ((?x21 (bvshl ?x20 (_ bv24 32))))
 (let ((?x22 (bvashr ?x21 (_ bv24 32))))
 (let (($x29 (= ?x22 (_ bv0 32))))
 (not $x29)))))))
(assert
 (let ((?x24 ((_ extract 7 0) symbol_7)))
 (let ((?x25 ((_ zero_extend 24) ?x24)))
 (let ((?x26 (bvshl ?x25 (_ bv24 32))))
 (let ((?x27 (bvashr ?x26 (_ bv24 32))))
 (let ((?x32 ((_ extract 7 0) symbol_3)))
 (let ((?x33 ((_ zero_extend 24) ?x32)))
 (let ((?x34 (bvshl ?x33 (_ bv24 32))))
 (let ((?x35 (bvashr ?x34 (_ bv24 32))))
 (and (distinct ?x35 ?x27) true))))))))))
(assert
 (let ((?x24 ((_ extract 7 0) symbol_7)))
 (let ((?x25 ((_ zero_extend 24) ?x24)))
 (let ((?x26 (bvshl ?x25 (_ bv24 32))))
 (let ((?x27 (bvashr ?x26 (_ bv24 32))))
 (let ((?x38 ((_ extract 7 0) symbol_2)))
 (let ((?x39 ((_ zero_extend 24) ?x38)))
 (let ((?x40 (bvshl ?x39 (_ bv24 32))))
 (let ((?x41 (bvashr ?x40 (_ bv24 32))))
 (and (distinct ?x41 ?x27) true))))))))))
(assert
 (let ((?x24 ((_ extract 7 0) symbol_7)))
 (let ((?x25 ((_ zero_extend 24) ?x24)))
 (let ((?x26 (bvshl ?x25 (_ bv24 32))))
 (let ((?x27 (bvashr ?x26 (_ bv24 32))))
 (let ((?x44 ((_ extract 7 0) symbol_0)))
 (let ((?x45 ((_ zero_extend 24) ?x44)))
 (let ((?x46 (bvshl ?x45 (_ bv24 32))))
 (let ((?x47 (bvashr ?x46 (_ bv24 32))))
 (and (distinct ?x47 ?x27) true))))))))))
(assert
 (let ((?x19 ((_ extract 7 0) symbol_1)))
 (let ((?x20 ((_ zero_extend 24) ?x19)))
 (let ((?x21 (bvshl ?x20 (_ bv24 32))))
 (let ((?x22 (bvashr ?x21 (_ bv24 32))))
 (let ((?x32 ((_ extract 7 0) symbol_3)))
 (let ((?x33 ((_ zero_extend 24) ?x32)))
 (let ((?x34 (bvshl ?x33 (_ bv24 32))))
 (let ((?x35 (bvashr ?x34 (_ bv24 32))))
 (and (distinct ?x35 ?x22) true))))))))))
(assert
 (let ((?x32 ((_ extract 7 0) symbol_3)))
 (let ((?x33 ((_ zero_extend 24) ?x32)))
 (let ((?x34 (bvshl ?x33 (_ bv24 32))))
 (let ((?x35 (bvashr ?x34 (_ bv24 32))))
 (let (($x50 (= ?x35 (_ bv0 32))))
 (not $x50)))))))
(assert
 (let ((?x32 ((_ extract 7 0) symbol_3)))
 (let ((?x33 ((_ zero_extend 24) ?x32)))
 (let ((?x34 (bvshl ?x33 (_ bv24 32))))
 (let ((?x35 (bvashr ?x34 (_ bv24 32))))
 (let ((?x38 ((_ extract 7 0) symbol_2)))
 (let ((?x39 ((_ zero_extend 24) ?x38)))
 (let ((?x40 (bvshl ?x39 (_ bv24 32))))
 (let ((?x41 (bvashr ?x40 (_ bv24 32))))
 (and (distinct ?x41 ?x35) true))))))))))
(assert
 (let ((?x44 ((_ extract 7 0) symbol_0)))
 (let ((?x45 ((_ zero_extend 24) ?x44)))
 (let ((?x46 (bvshl ?x45 (_ bv24 32))))
 (let ((?x47 (bvashr ?x46 (_ bv24 32))))
 (let ((?x38 ((_ extract 7 0) symbol_2)))
 (let ((?x39 ((_ zero_extend 24) ?x38)))
 (let ((?x40 (bvshl ?x39 (_ bv24 32))))
 (let ((?x41 (bvashr ?x40 (_ bv24 32))))
 (and (distinct ?x41 ?x47) true))))))))))
(assert
 (let ((?x19 ((_ extract 7 0) symbol_1)))
 (let ((?x20 ((_ zero_extend 24) ?x19)))
 (let ((?x21 (bvshl ?x20 (_ bv24 32))))
 (let ((?x22 (bvashr ?x21 (_ bv24 32))))
 (let ((?x38 ((_ extract 7 0) symbol_2)))
 (let ((?x39 ((_ zero_extend 24) ?x38)))
 (let ((?x40 (bvshl ?x39 (_ bv24 32))))
 (let ((?x41 (bvashr ?x40 (_ bv24 32))))
 (and (distinct ?x41 ?x22) true))))))))))
(assert
 (let ((?x38 ((_ extract 7 0) symbol_2)))
 (let ((?x39 ((_ zero_extend 24) ?x38)))
 (let ((?x40 (bvshl ?x39 (_ bv24 32))))
 (let ((?x41 (bvashr ?x40 (_ bv24 32))))
 (let (($x55 (= ?x41 (_ bv0 32))))
 (not $x55)))))))
(assert
 (let ((?x19 ((_ extract 7 0) symbol_1)))
 (let ((?x20 ((_ zero_extend 24) ?x19)))
 (let ((?x21 (bvshl ?x20 (_ bv24 32))))
 (let ((?x22 (bvashr ?x21 (_ bv24 32))))
 (let ((?x44 ((_ extract 7 0) symbol_0)))
 (let ((?x45 ((_ zero_extend 24) ?x44)))
 (let ((?x46 (bvshl ?x45 (_ bv24 32))))
 (let ((?x47 (bvashr ?x46 (_ bv24 32))))
 (and (distinct ?x47 ?x22) true))))))))))
(assert
 (let ((?x44 ((_ extract 7 0) symbol_0)))
 (let ((?x45 ((_ zero_extend 24) ?x44)))
 (let ((?x46 (bvshl ?x45 (_ bv24 32))))
 (let ((?x47 (bvashr ?x46 (_ bv24 32))))
 (let (($x58 (= ?x47 (_ bv0 32))))
 (not $x58)))))))
(check-sat)
