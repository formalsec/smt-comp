; query-00031.smtml
(set-info :status unknown)
(set-logic ALL)
(declare-fun symbol_7 () (_ BitVec 32))
(declare-fun symbol_3 () (_ BitVec 32))
(declare-fun symbol_2 () (_ BitVec 32))
(declare-fun symbol_0 () (_ BitVec 32))
(declare-fun symbol_1 () (_ BitVec 32))
(assert
 (let ((?x17 ((_ extract 7 0) symbol_7)))
 (let ((?x18 ((_ zero_extend 24) ?x17)))
 (let ((?x19 (bvshl ?x18 (_ bv24 32))))
 (let ((?x20 (bvashr ?x19 (_ bv24 32))))
 (let ((?x11 ((_ extract 7 0) symbol_3)))
 (let ((?x12 ((_ zero_extend 24) ?x11)))
 (let ((?x14 (bvshl ?x12 (_ bv24 32))))
 (let ((?x15 (bvashr ?x14 (_ bv24 32))))
 (and (distinct ?x15 ?x20) true))))))))))
(assert
 (let ((?x17 ((_ extract 7 0) symbol_7)))
 (let ((?x18 ((_ zero_extend 24) ?x17)))
 (let ((?x19 (bvshl ?x18 (_ bv24 32))))
 (let ((?x20 (bvashr ?x19 (_ bv24 32))))
 (let ((?x23 ((_ extract 7 0) symbol_2)))
 (let ((?x24 ((_ zero_extend 24) ?x23)))
 (let ((?x25 (bvshl ?x24 (_ bv24 32))))
 (let ((?x26 (bvashr ?x25 (_ bv24 32))))
 (and (distinct ?x26 ?x20) true))))))))))
(assert
 (let ((?x17 ((_ extract 7 0) symbol_7)))
 (let ((?x18 ((_ zero_extend 24) ?x17)))
 (let ((?x19 (bvshl ?x18 (_ bv24 32))))
 (let ((?x20 (bvashr ?x19 (_ bv24 32))))
 (let ((?x29 ((_ extract 7 0) symbol_0)))
 (let ((?x30 ((_ zero_extend 24) ?x29)))
 (let ((?x31 (bvshl ?x30 (_ bv24 32))))
 (let ((?x32 (bvashr ?x31 (_ bv24 32))))
 (and (distinct ?x32 ?x20) true))))))))))
(assert
 (let ((?x35 ((_ extract 7 0) symbol_1)))
 (let ((?x36 ((_ zero_extend 24) ?x35)))
 (let ((?x37 (bvshl ?x36 (_ bv24 32))))
 (let ((?x38 (bvashr ?x37 (_ bv24 32))))
 (let ((?x11 ((_ extract 7 0) symbol_3)))
 (let ((?x12 ((_ zero_extend 24) ?x11)))
 (let ((?x14 (bvshl ?x12 (_ bv24 32))))
 (let ((?x15 (bvashr ?x14 (_ bv24 32))))
 (and (distinct ?x15 ?x38) true))))))))))
(assert
 (let ((?x11 ((_ extract 7 0) symbol_3)))
 (let ((?x12 ((_ zero_extend 24) ?x11)))
 (let ((?x14 (bvshl ?x12 (_ bv24 32))))
 (let ((?x15 (bvashr ?x14 (_ bv24 32))))
 (let (($x41 (= ?x15 (_ bv0 32))))
 (not $x41)))))))
(assert
 (let ((?x11 ((_ extract 7 0) symbol_3)))
 (let ((?x12 ((_ zero_extend 24) ?x11)))
 (let ((?x14 (bvshl ?x12 (_ bv24 32))))
 (let ((?x15 (bvashr ?x14 (_ bv24 32))))
 (let ((?x23 ((_ extract 7 0) symbol_2)))
 (let ((?x24 ((_ zero_extend 24) ?x23)))
 (let ((?x25 (bvshl ?x24 (_ bv24 32))))
 (let ((?x26 (bvashr ?x25 (_ bv24 32))))
 (and (distinct ?x26 ?x15) true))))))))))
(assert
 (let ((?x29 ((_ extract 7 0) symbol_0)))
 (let ((?x30 ((_ zero_extend 24) ?x29)))
 (let ((?x31 (bvshl ?x30 (_ bv24 32))))
 (let ((?x32 (bvashr ?x31 (_ bv24 32))))
 (let ((?x23 ((_ extract 7 0) symbol_2)))
 (let ((?x24 ((_ zero_extend 24) ?x23)))
 (let ((?x25 (bvshl ?x24 (_ bv24 32))))
 (let ((?x26 (bvashr ?x25 (_ bv24 32))))
 (and (distinct ?x26 ?x32) true))))))))))
(assert
 (let ((?x35 ((_ extract 7 0) symbol_1)))
 (let ((?x36 ((_ zero_extend 24) ?x35)))
 (let ((?x37 (bvshl ?x36 (_ bv24 32))))
 (let ((?x38 (bvashr ?x37 (_ bv24 32))))
 (let ((?x23 ((_ extract 7 0) symbol_2)))
 (let ((?x24 ((_ zero_extend 24) ?x23)))
 (let ((?x25 (bvshl ?x24 (_ bv24 32))))
 (let ((?x26 (bvashr ?x25 (_ bv24 32))))
 (and (distinct ?x26 ?x38) true))))))))))
(assert
 (let ((?x23 ((_ extract 7 0) symbol_2)))
 (let ((?x24 ((_ zero_extend 24) ?x23)))
 (let ((?x25 (bvshl ?x24 (_ bv24 32))))
 (let ((?x26 (bvashr ?x25 (_ bv24 32))))
 (let (($x46 (= ?x26 (_ bv0 32))))
 (not $x46)))))))
(assert
 (let ((?x35 ((_ extract 7 0) symbol_1)))
 (let ((?x36 ((_ zero_extend 24) ?x35)))
 (let ((?x37 (bvshl ?x36 (_ bv24 32))))
 (let ((?x38 (bvashr ?x37 (_ bv24 32))))
 (let ((?x29 ((_ extract 7 0) symbol_0)))
 (let ((?x30 ((_ zero_extend 24) ?x29)))
 (let ((?x31 (bvshl ?x30 (_ bv24 32))))
 (let ((?x32 (bvashr ?x31 (_ bv24 32))))
 (and (distinct ?x32 ?x38) true))))))))))
(assert
 (let ((?x29 ((_ extract 7 0) symbol_0)))
 (let ((?x30 ((_ zero_extend 24) ?x29)))
 (let ((?x31 (bvshl ?x30 (_ bv24 32))))
 (let ((?x32 (bvashr ?x31 (_ bv24 32))))
 (let (($x49 (= ?x32 (_ bv0 32))))
 (not $x49)))))))
(check-sat)
