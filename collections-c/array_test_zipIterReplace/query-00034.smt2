; query-00034.smtml
(set-info :status unknown)
(set-logic ALL)
(declare-fun symbol_7 () (_ BitVec 32))
(declare-fun symbol_1 () (_ BitVec 32))
(declare-fun symbol_3 () (_ BitVec 32))
(declare-fun symbol_2 () (_ BitVec 32))
(declare-fun symbol_0 () (_ BitVec 32))
(assert
 (let ((?x17 ((_ extract 7 0) symbol_7)))
 (let ((?x18 ((_ zero_extend 24) ?x17)))
 (let ((?x19 (bvshl ?x18 (_ bv24 32))))
 (let ((?x20 (bvashr ?x19 (_ bv24 32))))
 (let ((?x11 ((_ extract 7 0) symbol_1)))
 (let ((?x12 ((_ zero_extend 24) ?x11)))
 (let ((?x14 (bvshl ?x12 (_ bv24 32))))
 (let ((?x15 (bvashr ?x14 (_ bv24 32))))
 (let (($x21 (and (distinct ?x15 ?x20) true)))
 (not $x21)))))))))))
(assert
 (let ((?x11 ((_ extract 7 0) symbol_1)))
 (let ((?x12 ((_ zero_extend 24) ?x11)))
 (let ((?x14 (bvshl ?x12 (_ bv24 32))))
 (let ((?x15 (bvashr ?x14 (_ bv24 32))))
 (let (($x24 (= ?x15 (_ bv0 32))))
 (not $x24)))))))
(assert
 (let ((?x17 ((_ extract 7 0) symbol_7)))
 (let ((?x18 ((_ zero_extend 24) ?x17)))
 (let ((?x19 (bvshl ?x18 (_ bv24 32))))
 (let ((?x20 (bvashr ?x19 (_ bv24 32))))
 (let ((?x27 ((_ extract 7 0) symbol_3)))
 (let ((?x28 ((_ zero_extend 24) ?x27)))
 (let ((?x29 (bvshl ?x28 (_ bv24 32))))
 (let ((?x30 (bvashr ?x29 (_ bv24 32))))
 (and (distinct ?x30 ?x20) true))))))))))
(assert
 (let ((?x17 ((_ extract 7 0) symbol_7)))
 (let ((?x18 ((_ zero_extend 24) ?x17)))
 (let ((?x19 (bvshl ?x18 (_ bv24 32))))
 (let ((?x20 (bvashr ?x19 (_ bv24 32))))
 (let ((?x33 ((_ extract 7 0) symbol_2)))
 (let ((?x34 ((_ zero_extend 24) ?x33)))
 (let ((?x35 (bvshl ?x34 (_ bv24 32))))
 (let ((?x36 (bvashr ?x35 (_ bv24 32))))
 (and (distinct ?x36 ?x20) true))))))))))
(assert
 (let ((?x17 ((_ extract 7 0) symbol_7)))
 (let ((?x18 ((_ zero_extend 24) ?x17)))
 (let ((?x19 (bvshl ?x18 (_ bv24 32))))
 (let ((?x20 (bvashr ?x19 (_ bv24 32))))
 (let ((?x39 ((_ extract 7 0) symbol_0)))
 (let ((?x40 ((_ zero_extend 24) ?x39)))
 (let ((?x41 (bvshl ?x40 (_ bv24 32))))
 (let ((?x42 (bvashr ?x41 (_ bv24 32))))
 (and (distinct ?x42 ?x20) true))))))))))
(assert
 (let ((?x11 ((_ extract 7 0) symbol_1)))
 (let ((?x12 ((_ zero_extend 24) ?x11)))
 (let ((?x14 (bvshl ?x12 (_ bv24 32))))
 (let ((?x15 (bvashr ?x14 (_ bv24 32))))
 (let ((?x27 ((_ extract 7 0) symbol_3)))
 (let ((?x28 ((_ zero_extend 24) ?x27)))
 (let ((?x29 (bvshl ?x28 (_ bv24 32))))
 (let ((?x30 (bvashr ?x29 (_ bv24 32))))
 (and (distinct ?x30 ?x15) true))))))))))
(assert
 (let ((?x27 ((_ extract 7 0) symbol_3)))
 (let ((?x28 ((_ zero_extend 24) ?x27)))
 (let ((?x29 (bvshl ?x28 (_ bv24 32))))
 (let ((?x30 (bvashr ?x29 (_ bv24 32))))
 (let (($x45 (= ?x30 (_ bv0 32))))
 (not $x45)))))))
(assert
 (let ((?x27 ((_ extract 7 0) symbol_3)))
 (let ((?x28 ((_ zero_extend 24) ?x27)))
 (let ((?x29 (bvshl ?x28 (_ bv24 32))))
 (let ((?x30 (bvashr ?x29 (_ bv24 32))))
 (let ((?x33 ((_ extract 7 0) symbol_2)))
 (let ((?x34 ((_ zero_extend 24) ?x33)))
 (let ((?x35 (bvshl ?x34 (_ bv24 32))))
 (let ((?x36 (bvashr ?x35 (_ bv24 32))))
 (and (distinct ?x36 ?x30) true))))))))))
(assert
 (let ((?x39 ((_ extract 7 0) symbol_0)))
 (let ((?x40 ((_ zero_extend 24) ?x39)))
 (let ((?x41 (bvshl ?x40 (_ bv24 32))))
 (let ((?x42 (bvashr ?x41 (_ bv24 32))))
 (let ((?x33 ((_ extract 7 0) symbol_2)))
 (let ((?x34 ((_ zero_extend 24) ?x33)))
 (let ((?x35 (bvshl ?x34 (_ bv24 32))))
 (let ((?x36 (bvashr ?x35 (_ bv24 32))))
 (and (distinct ?x36 ?x42) true))))))))))
(assert
 (let ((?x11 ((_ extract 7 0) symbol_1)))
 (let ((?x12 ((_ zero_extend 24) ?x11)))
 (let ((?x14 (bvshl ?x12 (_ bv24 32))))
 (let ((?x15 (bvashr ?x14 (_ bv24 32))))
 (let ((?x33 ((_ extract 7 0) symbol_2)))
 (let ((?x34 ((_ zero_extend 24) ?x33)))
 (let ((?x35 (bvshl ?x34 (_ bv24 32))))
 (let ((?x36 (bvashr ?x35 (_ bv24 32))))
 (and (distinct ?x36 ?x15) true))))))))))
(assert
 (let ((?x33 ((_ extract 7 0) symbol_2)))
 (let ((?x34 ((_ zero_extend 24) ?x33)))
 (let ((?x35 (bvshl ?x34 (_ bv24 32))))
 (let ((?x36 (bvashr ?x35 (_ bv24 32))))
 (let (($x50 (= ?x36 (_ bv0 32))))
 (not $x50)))))))
(assert
 (let ((?x11 ((_ extract 7 0) symbol_1)))
 (let ((?x12 ((_ zero_extend 24) ?x11)))
 (let ((?x14 (bvshl ?x12 (_ bv24 32))))
 (let ((?x15 (bvashr ?x14 (_ bv24 32))))
 (let ((?x39 ((_ extract 7 0) symbol_0)))
 (let ((?x40 ((_ zero_extend 24) ?x39)))
 (let ((?x41 (bvshl ?x40 (_ bv24 32))))
 (let ((?x42 (bvashr ?x41 (_ bv24 32))))
 (and (distinct ?x42 ?x15) true))))))))))
(assert
 (let ((?x39 ((_ extract 7 0) symbol_0)))
 (let ((?x40 ((_ zero_extend 24) ?x39)))
 (let ((?x41 (bvshl ?x40 (_ bv24 32))))
 (let ((?x42 (bvashr ?x41 (_ bv24 32))))
 (let (($x53 (= ?x42 (_ bv0 32))))
 (not $x53)))))))
(check-sat)
